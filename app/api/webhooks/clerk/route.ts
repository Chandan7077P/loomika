...
const CLERK_WEBHOOK_SECRET = process.env.CLERK_WEBHOOK_SECRET || 'YOUR_CLERK_WEBHOOK_SECRET';
...
export async function POST(req: Request) {
  ...
  if (eventType === 'user.created') {
    ...
    if (!email_addresses || email_addresses.length === 0) {
      ...
      return new NextResponse('User created event missing email address', { status: 400 });
    }
